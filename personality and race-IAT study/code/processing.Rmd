...
table(data_age_gen$gender, useNA = "ifany") |>  #checking the values in gender and act accordingly
  kable()  |>
...
map(data_age_gen, class) # checking the class for age and gender
```
...
for (i in seq_along(subscales)) {
  ...
  dat_subscale <- data_bfi_rs %>% select(selected_columns) # Select columns for the current subscale
  
  
  cor_matrix <- cor(dat_subscale, use = "pairwise.complete.obs") # Compute correlation matrix
  
  
  cor_matrices[[subscales[i]]] <- cor_matrix # Store the correlation matrix in the list
}
...
for (i in seq_along(subscales)) {
    ...
    dat_subscale <- data_bfi_rs |>
      select(subject, selected_columns)
...
for (i in seq_along(subscales)) {
  ...
  for (subject in started_subjects) {
    # Extract responses for this subject and subscale
    subject_responses <- dat_subscale[dat_subscale$subject == subject, -1]

    # Check for values outside the 1-6 range
    if (any(subject_responses < 1 | subject_responses > 6, na.rm = TRUE)) {
      subjects_outside_range[[subscales[i]]] <- c(subjects_outside_range[[subscales[i]]], subject)
    }

    # Check for NA values
    if (any(is.na(subject_responses))) {
...
mean_score_bfi <- data_bfi_check %>%
  group_by(subject) %>%
  summarize(
    mean_a = calculate_mean_subscale(cur_data(), "a"),
...
# View the mean scores per subject
# view(mean_score_bfi)
...
check_mean_range <- function(mean_score, min, max) {
  ifelse(mean_score >= min & mean_score <= max, "Valid", "Invalid")
}

# Apply the range check to each mean subscale score for all subjects in mean_score_bfi
mean_score_bfi <- mean_score_bfi %>%
  mutate(
    check_mean_a = check_mean_range(mean_a, min_max_a$min, min_max_a$max),
...
# View the updated dataframe with range checks
# view(mean_score_bfi)
...
proportion_rt_acc <- data_iat_var |>
  ...
  mutate(exclude_performance = ifelse(proportion_fast_trials > 0.10| proportion_accuracy < 0.75, "exclude", "include"))
...
# Join the include_exclude_iat column
...
data_processed_temp <- data_age_gen |> 
  inner_join(data_bfi_check, by = "subject") |> 
