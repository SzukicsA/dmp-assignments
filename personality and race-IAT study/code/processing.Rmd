...
dat_age_gender <- data_dem_raw |>
  pivot_wider(names_from  = variable,
              values_from = response) |> 
  rename(subject          = unique_id,
         gender           = sex)      |> 
  mutate(across(c(age, gender), ~map(.x, unlist))) #|>
  # mutate(gender = tolower(gender),
  #        gender = stringr::str_remove_all(gender, regex("\\W+")), # regex is both very useful and awful to write
  #        gender = case_when(gender == "f" ~ gender,
  #                           gender == "m" ~ gender,
  #                           age == "nonbinary" ~ gender,
  #                           age == "woman" ~ "female",
  #                           TRUE ~ "other/missing/error"),
  #        age = case_when(str_detect(age, "^[0-9]+$") ~ age, # if the value is only numbers, keep it.
  #                        TRUE ~ "other/missing/error"))

# Assuming is your data frame and 'sex' is the column
unique(dat_age_gender$gender)

class(dat_age_gender$subject)

class(dat_age_gender$gender)

class(dat_age_gender$age)
