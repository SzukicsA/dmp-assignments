...
for (i in seq_along(subscales)) {
  ...
  dat_subscale <- data_bfi_rs %>% select(subject, selected_columns)  # Select columns for the current subscale
  completeness_subject <- rowSums(!is.na(dat_subscale[, -1])) == num_items[i] # Check completeness for each subject
  completeness_info[[subscales[i]]] <- data.frame(subject = dat_subscale$subject, complete = completeness_subject) # Store completeness information in the list
  }
...
# Check if at least 3 subscales are complete for each subject
completeness_combined$at_least_3_complete <- rowSums(completeness_combined[, -1], na.rm = TRUE) >= 3

# Create a new column "completeness_check" in dat_bfi_rs
data_bfi_rs$completeness_check <- ifelse(data_bfi_rs$subject %in% completeness_combined$subject[completeness_combined$at_least_3_complete], "include", "exclude")

# Print or use the dat_bfi_rs with the new completeness_check column as needed
view(data_bfi_rs)
